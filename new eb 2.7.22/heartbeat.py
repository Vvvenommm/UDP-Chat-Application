...
def start_heartbeat():
    while True:
        # create the Socket for Heartbeat
        ...
        ring = leader_election.form_ring(utils.SERVER_LIST)
        my_neighbour = leader_election.get_neighbour(ring, utils.myIP, 'left')
        ...
        if my_neighbour != "":
            ...
            # if connecting to Neighbour was not possible, the Heartbeat failed -> Neighbour crashed
                ...
                if utils.leader == my_neighbour:
                    print(f'[HEARTBEAT] Server Leader {utils.leader} crashed',
                          file=sys.stderr)
