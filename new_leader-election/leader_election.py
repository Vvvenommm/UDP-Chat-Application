...
def start_election(no_existing_leader):
    ...
    election_message = pickle.dumps([data, addr])
    ...
    if election_message[0] < utils.myIP and not participant:  # wenn mIP von election_message kl. als meine eigene IP Adresse, ändere election_message ab
        no_existing_leader = False
    ...
    elif election_message[0] > utils.myIP:
        # send received election message to left neighbour
        participant = True
        ring_socket.sendto(election_message, neighbour)
